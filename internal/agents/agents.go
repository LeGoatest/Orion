...
func (j *AgentJob) Execute(ctx context.Context) error {
	...
	if err != nil {
		j.EB.Publish(types.Event{Type: "agent.error", GoalID: j.GoalID, Stage: j.Stage, Payload: map[string]string{"agent": j.Agent.Name(), "error": err.Error()}, CreatedAt: time.Now()})
		return err
	}
...
func (s *Supervisor) StartAgents(ctx context.Context) error {
	for _, a := range s.Reg.List() {
		s.EB.Publish(types.Event{Type: "agent.started", Payload: a.Name(), CreatedAt: time.Now()})
	}
	return nil
}
...
func (d *Dispatcher) Dispatch(ctx context.Context, stage, goalID string, task interface{}) error {
	m := d.Reg.GetByCap(stage)
	if len(m) == 0 {
		return fmt.Errorf("no agent for %s", stage)
	}
	d.Sch.Schedule(&AgentJob{IDVal: uuid.New().String(), GoalID: goalID, Stage: stage, Agent: m[0], Task: task, EB: d.EB})
	return nil
}
