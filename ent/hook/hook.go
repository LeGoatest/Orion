...
// The GoalEventFunc type is an adapter to allow the use of ordinary
// function as GoalEvent mutator.
type GoalEventFunc func(context.Context, *ent.GoalEventMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f GoalEventFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.GoalEventMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.GoalEventMutation", m)
}
